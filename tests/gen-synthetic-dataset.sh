#!/usr/bin/env bash
# Generate a synthetic ICAO 9303 e2e test dataset.
#
# Creates: CSCA cert, DS cert (signed by CSCA), and EF_SOD.bin (CMS-signed
# LdsSecurityObject committing SHA-256 hashes of the BSI DG files).
#
# Requirements: OpenSSL 3.x, Python 3
#
# Usage: ./tests/gen-synthetic-dataset.sh [output_dir]
#   Default output: tests/dataset-synth/

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
OUT_DIR="${1:-$REPO_ROOT/tests/dataset-synth}"
WORK="$(mktemp -d)"
trap 'rm -rf "$WORK"' EXIT

BSI_DATASET="$REPO_ROOT/tests/dataset"

echo "==> Generating synthetic ICAO 9303 dataset"
echo "    BSI DGs:  $BSI_DATASET"
echo "    Output:   $OUT_DIR"
echo "    Work dir: $WORK"

# ── 1. Generate CSCA (Country Signing CA) ────────────────────────────────────
echo "--- Generating CSCA key + self-signed cert (RSA 2048, RSA-PSS-SHA256) ---"
openssl genrsa -out "$WORK/csca-key.pem" 2048 2>/dev/null
openssl req -new -x509 \
    -key "$WORK/csca-key.pem" \
    -out "$WORK/csca.pem" \
    -days 3650 \
    -subj "/C=ZZ/O=Synthetic CSCA/CN=Test Country Signing CA" \
    -sha256 \
...
echo "--- Generating DS key + cert signed by CSCA (RSA 2048, RSA-PSS-SHA256) ---"
openssl genrsa -out "$WORK/ds-key.pem" 2048 2>/dev/null
openssl req -new \
    -key "$WORK/ds-key.pem" \
    -out "$WORK/ds.csr" \
    -subj "/C=ZZ/O=Synthetic DS/CN=Test Document Signer" \
    -sha256 \
    -sigopt rsa_padding_mode:pss \
    -sigopt rsa_pss_saltlen:32 \
    2>/dev/null
...
# ── 4. CMS sign the LdsSecurityObject with DS key ───────────────────────────
echo "--- CMS signing LdsSecurityObject (RSA-PSS-SHA256, ICAO content type) ---"
...
echo ""
echo "==> Synthetic dataset generated in $OUT_DIR/"
echo "    CSCA.cer    - Self-signed Country Signing CA (RSA 2048)"
echo "    DSC.cer     - Document Signer cert signed by CSCA"
